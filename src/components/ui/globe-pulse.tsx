...
export function GlobePulse({
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      globe = createGlobe(canvas, {
        ...
        width, height: width,
        phi: 0, theta: 0.1, dark: 1, diffuse: 1.5,
        mapSamples: 16000, mapBrightness: 10,
        baseColor: [0.3, 0.3, 0.3], // Darker continents for contrast
        markerColor: [0.2, 0.8, 0.9],
        glowColor: [0.03, 0.03, 0.03], // Slightly dark atmosphere
        markerElevation: 0,
      ...
      function animate() {
        ...
        globe!.update({
          ...
          theta: 0.1 + thetaOffsetRef.current + dragOffset.current.theta,
        })
  ...
  return (
    <div className={`relative aspect-square select-none flex items-center justify-center ${className}`}>
      {/* ─── CUSTOM GLOWING RING BACKGROUND ─── */}
      <div 
        className="absolute inset-[10%] rounded-full bg-gradient-to-tr from-purple-600/70 via-transparent to-cyan-500/70 blur-2xl opacity-90 pointer-events-none" 
        style={{ transform: "scale(1.15)" }} 
      />
      <div className="absolute inset-0 rounded-full bg-gradient-to-b from-transparent via-transparent to-black z-0 opacity-40 pointer-events-none" />
      ...
      {markers.map((m) => (
        ...
        >
          ...
          <span style={{
            ...
            boxShadow: "0 0 0 2px #000, 0 0 0 4px #33ccdd, 0 0 10px 4px #33ccdd",
          }} />
        </div>
      ))}
    </div>
